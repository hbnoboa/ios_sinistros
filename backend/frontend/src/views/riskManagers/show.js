...
const RiskManagerShow = () => {
  ...
  const onDelete = async () => {
    if (!window.confirm("Deseja realmente excluir este gestor?")) return;
    try {
      const res = await fetch(`${API}/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}`, "x-tenant-id": tenant },
  ...
  if (!item) return <Alert variant="warning">Gestor não encontrado.</Alert>;
...
export default RiskManagerShow;
